...
gryadki = int( input( 'Количетсво грядок ' ) )
lister = [ int(x) for x in input( 'урожай ' ).split() ]
gryadki = len(lister)
lister = lister + lister[:2]
count = 0
for i in range(gryadki):
   count = max( count, lister[i] + lister[i+1] + lister[i+2] )
print(count)
